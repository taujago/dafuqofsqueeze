<?php 
class formulir_pindah_datang_antar_desa extends op_controller {
    function __construct(){
        parent::__construct();
        $this->load->model("core_model","cm");
        $this->load->model("m_formulir_pindah_datang_antar_desa","dm");
    ...
    function index(){
        ...
        $data['header'] = "Data Formulir Permohonan Pindah Datang WNI Antar Desa/Kelurahan Dalam Satu Kecamatan (F-1.27)";
    ...
    function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['dusun_formulir']           = $result[$i]['dusun_formulir'] ;
            ...
            $responce->rows[$i]['kepala_keluarga_alamat_sebelumnya']          = $result[$i]['kepala_keluarga_alamat_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_rt_sebelumnya']          = $result[$i]['kepala_keluarga_rt_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_rw_sebelumnya']          = $result[$i]['kepala_keluarga_rw_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_desa_sebelumnya']          = $result[$i]['kepala_keluarga_desa_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_kecamatan_sebelumnya']          = $result[$i]['kepala_keluarga_kecamatan_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_kota_sebelumnya']          = $result[$i]['kepala_keluarga_kota_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_provinsi_sebelumnya']          = $result[$i]['kepala_keluarga_provinsi_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_dusun_sebelumnya']          = $result[$i]['kepala_keluarga_dusun_sebelumnya'] ; 
            $responce->rows[$i]['kepala_keluarga_telepon']          = $result[$i]['kepala_keluarga_telepon'] ; 

             $responce->rows[$i]['kepala_keluarga_pos_sebelumnya']          = $result[$i]['kepala_keluarga_pos_sebelumnya'] ; 
             ...
             $responce->rows[$i]['tanggal_kedatangan']      = flipdate($result[$i]['tanggal_kedatangan']) ;
            ...
            $responce->rows[$i]['tujuan_nik']          = $result[$i]['tujuan_nik'] ; 
            $responce->rows[$i]['tujuan_nama']          = $result[$i]['tujuan_nama'] ; 

            $responce->rows[$i]['tujuan_no_kk1']       = $result[$i]['tujuan_no_kk1'];     
            $responce->rows[$i]['tujuan_alamat']  = $result[$i]['tujuan_alamat'] ;             
            $responce->rows[$i]['tujuan_rt']  = $result[$i]['tujuan_rt'] ;             
            $responce->rows[$i]['tujuan_rw']          = $result[$i]['tujuan_rw'] ; 
            $responce->rows[$i]['tujuan_dusun']          = $result[$i]['tujuan_dusun'] ; 
            $responce->rows[$i]['tujuan_telpon']          = $result[$i]['tujuan_telpon'] ; 
    ...
    function save(){
        ...
        $this->form_validation->set_rules('kepala_keluarga_nik','Nik Kepala Keluarga','required');  
        $this->form_validation->set_rules('no_formulir','Nomor Formulir ','required');
        $this->form_validation->set_rules('status','Status No. KK Bagi Yang Datang','required');              
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $data['tanggal'] = date("Y-m-d");
                 $data['tanggal_kedatangan'] = flipdate($data['tanggal_kedatangan']);
               ...
               // $arr_data_pengikut['id_penduduk'] = $data['id_penduduk_pengikut'];
                ...
                $res = $this->db->insert("formulir_pindah_datang_antar_desa",$data);
                ...
                $arr_data_pengikut_session = $this->session->userdata("arr_pindah_datang");
                //echo "<pre>";
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    ...
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                       ...
                       $res2=  $this->db->insert("formulir_pindah_datang_antar_desa_detail",$arr_tmp);
                    ...
                    $this->session->unset_userdata("arr_pindah_datang");
                }
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
             $data['tanggal_kedatangan'] = flipdate($data['tanggal_kedatangan']);
                ...
                $xy = $this->db->get("formulir_pindah_datang_antar_desa")->row();
                // $this->db->where("id_penduduk",$xy->id_penduduk);
                // $this->db->update("penduduk",array("status_kependudukan"=>"0"));
                 
                $this->db->where("id",$data['id']);
                $res = $this->db->update("formulir_pindah_datang_antar_desa",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                ...
                $this->db->where("id",$data['id']);
                $this->db->delete("formulir_pindah_datang_antar_desa_detail");
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    $i=0;
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                        $i++;
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("formulir_pindah_datang_antar_desa");
    }
    echo json_encode(array("success"=>true,"pesan"=>"Berhasil dihapus"));
}
...
function cetak($id) {
   ...
   $this->db->select('*')->from('formulir_pindah_datang_antar_desa')
    ...
    $this->db->select("*")->from("formulir_pindah_datang_antar_desa_detail spd")
    ...
    $masa = tgl_indo(flipdate($masa_berlaku_ktp));
