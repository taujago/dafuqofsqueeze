...
class cetak_profil extends CI_Model 
{
	...
	function get_data_1()
	{
		$this->db->select('p.*,k.*, pd.pendidikan, 
			master_akseptor_kb.akseptor_kb,
			master_status_kepemilikan_rumah.status_kepemilikan_rumah,
			master_penghasilan.penghasilan,
			master_pengeluaran.pengeluaran, 
			m1.kepemilikan_lahan as m1,
			m2.kepemilikan_lahan as m2, 


			m.kepemilikan_lahan,
			mi.alat_produksi_ikan,
			ma.sumber_air,
			mk.kualitas_air, 
		...
		$this->db->join("kk_anggota  k","p.id_penduduk = k.id_penduduk","left");
		$this->db->join("pendidikan pd","p.id_pendidikan = pd.id_pendidikan","left");
