<?php 
class admin_a9 extends op_controller {
    var $desa2;
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("a9m","dm");
	...
	function index(){
		...
		$data['header'] = "Data Buku Lembaran Desa dan Berita Desa  " . $desa2->bentuk_lembaga;
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"tanggal_ditetapkan"; // get index row - i.e. user click to sort 
       ...
       /*$nama_ibu = isset($_REQUEST['search_nama_ibu'])?$_REQUEST['search_nama_ibu']:"x";
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['nomor_diundangkan']           = $result[$i]['nomor_diundangkan'] ;
            $responce->rows[$i]['tanggal_diundangkan']           = flipdate($result[$i]['tanggal_diundangkan']) ;
            $responce->rows[$i]['jenis']           = $result[$i]['jenis'] ;
            $responce->rows[$i]['nomor_ditetapkan']           = $result[$i]['nomor_ditetapkan'] ;
            $responce->rows[$i]['tanggal_ditetapkan']           = flipdate($result[$i]['tanggal_ditetapkan']) ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('nomor_ditetapkan','nomor_ditetapkan ','required');    
        $this->form_validation->set_rules('tanggal_ditetapkan','tanggal_ditetapkan ','required');              
        $this->form_validation->set_rules('jenis','jenis ','required');              
                  
        $this->form_validation->set_rules('nomor_diundangkan','Nomor Diundangkan ','required');              
        $this->form_validation->set_rules('tanggal_diundangkan','Tanggal Diundangkan ','required');              
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $data['tanggal_ditetapkan'] = flipdate($data['tanggal_ditetapkan']);
              //  $data['persetujuan_bpd_tgl'] = flipdate($data['persetujuan_bpd_tgl']);
                $data['tanggal_diundangkan'] = flipdate($data['tanggal_diundangkan']);
                $data['keterangan'] = ($data['keterangan']);

                $data['id_desa'] = $this->session->userdata("id_desa");
                $res = $this->db->insert("admin_a9",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("admin_a9",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->update("admin_a9",array("deleted"=>1));
    }
...
function pdf($tahun){
        ...
        $data['record'] = $this->dm->get_data_per_tahun($tahun);
        $data['tahun'] = $tahun;
        $data['header'] = "Lembaran Desa ".$this->desa2->bentuk_lembaga;
        $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/admin_a9_pdf_view",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
           ...
           // $html.="<h1> beda page </h1>";
            $pdf->AddPage();
            $html = $this->load->view("pdf/admin_a9_pdf_view_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
...
function excel($tahun) {
        ...
        $this->excel->getActiveSheet()->setTitle('Buku Lembaran '. $this->desa2->desa);
        ...
        ->setCellValue('A' . $baris,"BUKU LEMBARAN  ". strtoupper($this->desa2->bentuk_lembaga) ." DAN BERITA " . strtoupper($this->desa2->bentuk_lembaga));
        $baris++;
...
$baris +=3;
  ...
  $this->excel->getActiveSheet()
        ->setCellValue('f5', "MODEL A.9 ");  


$this->excel->getActiveSheet()
        ->setCellValue('A' . $baris, "No.")
        ->setCellValue('B' . $baris, "JENIS PERTURAN DI DESA")
        ->setCellValue('C' . $baris, "NOMOR DAN TANGGAL DITETAPKAN")
        ->setCellValue('D' . $baris, "TENTANG ")
         ->setCellValue('e' . $baris, "NOMOR DAN TANGGAL DIUNDANGKAN")
...
foreach($record->result() as $row) : 
    ...
    $this->excel->getActiveSheet()
        ...
        ->setCellValue('B' . $baris, $row->jenis)
        ->setCellValue('C' . $baris, "Nomor : $row->nomor_ditetapkan \nTanggal : $row->tanggal_ditetapkan")
        ->setCellValue('D' . $baris, $row->tentang)
         ->setCellValue('E' . $baris,"Nomor :  $row->nomor_diundangkan \nTanggal : $row->tanggal_diundangkan")
...
//$this->format(array("arr_kolom"=>$arr_kolom,"bold"=>true,"baris"=>$baris,"align"=>"center"));
        ...
        $filename = "BUKU LEMBARAN ".strtoupper($this->desa2->bentuk_lembaga).".xls";
