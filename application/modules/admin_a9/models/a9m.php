...
class a9m extends CI_Model 
{
	...
	{		
		...
		->where("deleted","0")->where("id_desa",$this->session->userdata("operator_id_desa"));
 		...
 		if($param['tgl_awal'] <> 'x' and $param['tgl_akhir'] <> 'x') {
 			...
 			$this->db->where(" tanggal_ditetapkan between '$awal' and '$akhir'",null,false);
 		}   
	...
	function get_data_per_tahun($tahun) {
		$this->db->where("year(tanggal_ditetapkan) = $tahun ",null,false);
		$this->db->where("id_desa",$this->session->userdata("operator_id_desa"));
		$this->db->where("deleted","0");
    	$this->db->order_by("tanggal_ditetapkan");
    	$res = $this->db->get("admin_a9");
    	return $res;
	}
