<?php 
class surat_rekomendasi_dana extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_rekomendasi_dana","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat Rekmendasi Pencairan Dana";
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                if($data['ttd'] == "camat" ) {
                    ...
                    if($desa->bentuk_lembaga=="Kelurahan") {
                        ...
                        ///$arr_lahir['nip_camat'] = $desa->nip_camat;
                         $data['nip'] = $desa->nip_camat;
                         $data['pangkat'] = $desa->pangkat_camat;
                    }
                    else { 
                        $data['ttd_jabatan'] = "CAMAT ";
                    }
                    
                    $data['ttd'] = $data['ttd'];                    
                    $data['ttd_nama'] = $desa->nama_camat;
                    $data['nip'] = $desa->nip_camat;
                    $data['pangkat'] = $desa->pangkat_camat;
                }
                    ...
                    if($desa->bentuk_lembaga=="Kelurahan") {
                        ...
                        $data['pangkat'] = $desa->pangkat_sek_desa;
                         ...
                         $data['ttd_jabatan'] = "Sekretaris Camat";
                    }
                ...
                $res = $this->db->insert("surat_rekomendasi_dana",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $data['id_penduduk'] = $this->cm->get_id_penduduk($data['nik']);
                unset($data['nik']);
                $data['rincian'] = $form['rincian'] ;

                $this->db->where("id",$data['id']);
                $res = $this->db->update("surat_rekomendasi_dana",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_rekomendasi_dana");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_rekomendasi_dana sd ')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan umum";
...
function pdf($id) {
    $this->db->select('*')->from('surat_rekomendasi_dana')
        // ->join("v_penduduk2 p",'p.id_penduduk=sd.id_penduduk')
        ->where("id",$id);
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat Rekmendasi Pencairan Dana";
    $data['title'] = $data['header'];
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_rekomendasi_dana_pdf_view",$data,true);
...
function word($id) {
    ...
    $this->load->helper('download');
        force_download("surat keterangan umum.doc", $html);
  }
