...
// $( 'textarea#rincian' ).ckeditor();

// CKEDITOR.replace( 'textarea#rincian', {
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Rekomendasi Pencairan Dana');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/KCP") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Rekomendasi Pencairan Dana'); 
	$('#fm').form('load',row);						 
