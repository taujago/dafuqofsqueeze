...
class indeks_desa_membangun_model extends CI_Model 
{
...
function get_monografi(){
	$res = $this->db->get("indeks_desa_membangun"); 
...
function get_jumlah_mati($tahun) {
	$sql="SELECT COUNT(*) as jumlah FROM surat_kematian_1
		WHERE YEAR(tanggal)  = '$tahun' and  deleted='0'";
	$res = $this->db->query($sql); 
	return $res->row_array();
}
...
function get_jumlah_pindah($tahun) {
	$sql = "SELECT COUNT(id_penduduk)  AS jumlah FROM (
...
function get_penduduk_per_pendidikan(){
	$sql="SELECT p.id_pendidikan,pendidikan, COUNT(id_penduduk) jumlah  FROM pendidikan p 
				LEFT JOIN ( SELECT * FROM penduduk WHERE `hidup_mati` = '1' AND status_kependudukan <> '3'  ) pdk ON p.`id_pendidikan` = pdk.`id_pendidikan`
				WHERE p.`deleted` = 0 
