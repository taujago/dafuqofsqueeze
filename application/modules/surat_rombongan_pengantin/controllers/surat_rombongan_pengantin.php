<?php 
class surat_rombongan_pengantin extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("srpm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat keterangan rombongan pengantin";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['jumlah_keluarga']          = $result[$i]['jumlah_keluarga'] ; 
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_rombongan_pengantin",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_rombongan_pengantin",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_rombongan_pengantin");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_rombongan_pengantin sd ')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan rombongan_pengantin";
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_rombongan_pengantin_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
        ...
        force_download("surat keterangan rombongan pengantin.doc", $html);   
} 
}
?>