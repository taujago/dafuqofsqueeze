...
$( 'textarea#ket_surat' ).ckeditor();

CKEDITOR.replace( 'textarea#ket_surat', {
...
function baru(){
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/422.5") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit surat keterangan tidak mampu'); 
	$('#fm').form('load',row);						 
