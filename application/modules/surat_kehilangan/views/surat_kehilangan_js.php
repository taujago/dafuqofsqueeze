...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Kehilangan');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/474.4") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Surat Kehilangan'); 
	$('#fm').form('load',row);						 
