<?php 
class surat_suami_diluar_daerah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_suami_diluar_daerah","dm");
	...
	function index(){
		...
		$data['header'] = "Data Surat Keterangan Suami Diluar Daerah";
    ...
    function save(){
        ...
        $data['id'] = md5(date("Ymdhis"));
                ...
                $xx = $this->db->insert("surat_suami_diluar_daerah",$data);
     ...
     $data = $this->input->post();
        ...
        $data['istri_tanggal_lahir'] = flipdate($data['istri_tanggal_lahir']);
         $data['tanggal_surat'] = flipdate($data['tanggal_surat']);
                ...
                $xx = $this->db->update("surat_suami_diluar_daerah",$data);
                // echo $this->db->last_query();  
                // echo mysql_error();
                if($xx) {
                   //echo $this->db->last_query() . 
...
function hapus(){
    ...
    foreach($ids as $id) {
		$this->db->where("id",$id);
		$xx = $this->db->get("surat_suami_diluar_daerah")->row();
		
        $this->db->where("id",$id);
        $this->db->delete("surat_suami_diluar_daerah");
		//echo $this->db->last_query();
...
function cetak($id) {
    ...
    $data = $this->db->get("surat_suami_diluar_daerah");
    $data = $data->row_array();
     $data['controller'] = get_class($this);
    $data['header'] = "Detail Surat Kelahiran";
    $data['title'] = $data['header'];
...
function pdf($id){
    ...
    // echo "fuck off " ; exit;
       ...
       // $pdf->SetAutoPageBreak(true,10);
        ...
        $html = $this->load->view("pdf/surat_suami_diluar_daerah_pdf_view",$data,true);
...
function word($id) {
        $this->db->select('*')->from('surat_suami_diluar_daerah ')
