...
$("#sumber_data_pemilik").change(function(){
	if($(this).val() == "in") {
		$('.tr_in_pemilik').show();
		$('.tr_ex_pemilik').hide();
	}
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Pernyataan Putus Tanah');
			$('#fm').form('clear');
			...
			$('.tr_in_pemilik').hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SPUTN") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pernyataan Putus Tanah'); 
	$('#fm').form('load',row);						 
