<?php 
class formulir_ktp extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("fktp","dm");
	...
	function index(){
		...
		$data['header'] = "Formulir Permohonan KTP";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['kode_kabupaten']         = $result[$i]['kode_kabupaten']; 
            $responce->rows[$i]['kode_kecamatan']         = $result[$i]['kode_kecamatan']; 
            $responce->rows[$i]['ktp']         = $result[$i]['ktp']; 
            $responce->rows[$i]['alamat']       = $result[$i]['alamat']; 
             $responce->rows[$i]['baru']       = $result[$i]['baru']; 
              $responce->rows[$i]['perpanjangan']       = $result[$i]['perpanjangan'];  
               $responce->rows[$i]['penggantian']       = $result[$i]['penggantian']; 
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("formulir_ktp",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("formulir_ktp",$data);
...
function cetak($id) {
    $this->db->select('*')->from('formulir_ktp sd ')
...
function pdf($id) {
    $this->db->select('*')->from('formulir_ktp sd ')
        ->join("v_penduduk p",'p.id_penduduk=sd.id_penduduk')
        ->where("id",$id);
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/formulir_ktp_pdf_view",$data,true);
