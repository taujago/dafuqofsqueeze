...
class perkembangan_jumlah_keluarga extends op_controller {
	...
	function reload(){
		$q = "SELECT  
			SUM(IF(jk='L' and p.kepala_keluarga='1' ,1,0)) AS L , 
			SUM(IF(jk='P' and p.kepala_keluarga='1',1,0)) AS P         
			FROM penduduk p 
	...
	//	echo $this->db->last_query(); 
		...
		// $data['lk'] =( $data_lama['lk']==0)?100:   ($data_lama['lk']/$data['lk']) / $data_lama['lk'] * 100;

		$data['lk'] =( $data_lama['lk']==0)?100:   ($data_lama['lk']/$data['lk']) * 100;
		$data['pr'] =( $data_lama['pr']==0)?100:   ($data_lama['pr']/$data['pr']) * 100;
