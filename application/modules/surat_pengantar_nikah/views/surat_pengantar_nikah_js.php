...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat  Keterangan Izin Membangunan ');
			$('#fm').form('clear');
			$("#id").val('<?php echo $this->session->userdata("operator_id_desa")."."; ?>');
			url = '<?php echo site_url("$controller/save") ?>';  
			
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKPN") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat  Keterangan Izin Membangunan'); 
	$('#fm').form('load',row);						 
