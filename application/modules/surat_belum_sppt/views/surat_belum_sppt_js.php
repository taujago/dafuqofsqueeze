...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Belum memiliki SPPT');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKBSPPT") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Belum memiliki SPPT'); 
	$('#fm').form('load',row);						 
