<?php 
class formulir_pindah_datang_antar_kota extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_formulir_pindah_datang_antar_kota","dm");
	...
	function index(){
		...
		$data['header'] = "Data Formulir Permohonan Pindah Datang WNI Antar Kab/Kota Atau Antar Provinsi (F-1.38)";
    ...
    function save(){
		...
		$this->form_validation->set_rules('no_formulir','No Formulir ','required');
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               // $arr_data_pengikut['id_penduduk'] = $data['id_penduduk_pengikut'];
                ...
                $res = $this->db->insert("formulir_pindah_datang_antar_kota",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    ...
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                       ...
                       $res2=  $this->db->insert("formulir_pindah_datang_antar_kota_detail",$arr_tmp);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
             $data['tanggal_kedatangan'] = flipdate($data['tanggal_kedatangan']);
				...
				$xy = $this->db->get("formulir_pindah_datang_antar_kota")->row();
				// $this->db->where("id_penduduk",$xy->id_penduduk);
				// $this->db->update("penduduk",array("status_kependudukan"=>"0"));
				 
				$this->db->where("id",$data['id']);
                $res = $this->db->update("formulir_pindah_datang_antar_kota",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                ...
                $this->db->where("id",$data['id']);
                $this->db->delete("formulir_pindah_datang_antar_kota_detail");
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    $i=0;
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                        $i++;
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("formulir_pindah_datang_antar_kota");
    }
    echo json_encode(array("success"=>true,"pesan"=>"Berhasil dihapus"));
}
...
function cetak($id) {
   ...
   $this->db->select('*')->from('formulir_pindah_datang_antar_kota')
    ...
    $this->db->select("*")->from("formulir_pindah_datang_antar_kota_detail spd")
