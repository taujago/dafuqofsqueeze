...
function penduduk_sebelum($bulan) {
	...
	function penduduk_input($bulan) {
	 ...
	 $sql ="SELECT 
			...
			+ IFNULL(SUM(IF(warga_negara='WNI' AND jk='P',1,0)),0) + IFNULL(SUM(IF(warga_negara='WNA' AND jk='P',1,0)),0) AS total 
				...
				FROM penduduk  
				WHERE hidup_mati = '1' and status_kependudukan <= 2 
				and regdate < '$today'";
	...
	function get_mati($bulan) {
		...
		$sql = "SELECT 
			...
			IFNULL(SUM(IF(p.warga_negara='WNI' AND p.jk='l',1,0)),0) + IFNULL(SUM(IF(p.warga_negara='WNA' AND p.jk='l',1,0)),0) AS jml_l, 
			IFNULL(SUM(IF(p.warga_negara='WNI' AND p.jk='P',1,0)),0) + IFNULL(SUM(IF(p.warga_negara='WNA' AND p.jk='P',1,0)),0) AS jml_p , 
			IFNULL(SUM(IF(p.warga_negara='WNI' AND p.jk='l',1,0)),0) + IFNULL(SUM(IF(p.warga_negara='WNA' AND p.jk='l',1,0)),0) 
			+ IFNULL(SUM(IF(p.warga_negara='WNI' AND p.jk='P',1,0)),0) + IFNULL(SUM(IF(p.warga_negara='WNA' AND p.jk='P',1,0)),0) AS total 
			
			FROM penduduk p
			 JOIN surat_kematian_1 l ON p.id_penduduk = l.id_penduduk
			 WHERE p.hidup_mati = '0'
			AND MONTH(l.tgl_meninggal) = $bulan
			 AND YEAR(l.tgl_meninggal) = $tahun";
		 $res = $this->db->query($sql);
		 // echo $this->db->last_query(). mysql_error(); exit; 
		 return $res->row();
	}
	...
	function pendatang_antar_kota($bulan) {
	 ...
	 /*	$sql="SELECT 
		...
		$sql = "SELECT IFNULL(SUM(IF(warga_negara='WNI' AND jk='L',1,0)),0) AS wni_l ,
			...
			FROM ( 
			SELECT id, sp.id_penduduk, tanggal, tgl_pindah, jk,warga_negara FROM surat_pindah sp 
			JOIN penduduk p ON p.id_penduduk = sp.`id_penduduk` 
			UNION
			SELECT id_surat_detail, spd.id_penduduk,  sp.tanggal, tgl_pindah, p.jk,p.`warga_negara` FROM surat_pindah_detail spd 
			JOIN surat_pindah sp ON spd.id = sp.`id` 
			JOIN penduduk p ON spd.id_penduduk = p.`id_penduduk` ) xx 

			
			WHERE YEAR(xx.tgl_pindah) = '$tahun' AND MONTH(tgl_pindah)='$bulan'";	 
		$res = $this->db->query($sql);
