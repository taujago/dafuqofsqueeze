	...
	function __construct()
	{
	 ...
	 $sql ="SELECT 
			...
			+ IFNULL(SUM(IF(warga_negara='WNI' AND jk='P',1,0)),0) + IFNULL(SUM(IF(warga_negara='WNA' AND jk='P',1,0)),0) AS total 
				...
				FROM penduduk  
				WHERE hidup_mati = '1' and status_kependudukan <> '1'
				and MONTH(regdate) <= $bulan and YEAR(regdate) = $tahun
			AND id_penduduk NOT IN 
			(SELECT sk.id_penduduk FROM surat_kelahiran sk 
			 JOIN penduduk p ON p.id_penduduk = sk.id_penduduk
			WHERE sk.deleted='0'  AND MONTH(p.tgl_lahir) = $bulan AND YEAR(p.tgl_lahir) = $tahun 
			)";
	...
	$sql="SELECT 
	 ...
	 $sql ="SELECT 
			...
			+ IFNULL(SUM(IF(warga_negara='WNI' AND jk='P',1,0)),0) + IFNULL(SUM(IF(warga_negara='WNA' AND jk='P',1,0)),0) AS total 
				...
				FROM penduduk  
				WHERE hidup_mati = '1' AND status_kependudukan not in ('1','3') 
				and MONTH(regdate) <= $bulan and YEAR(regdate) = $tahun";
	 $res = $this->db->query($sql);
	// echo $this->db->last_query();
	 return $res->row();
