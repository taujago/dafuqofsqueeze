<?php 
class statistik_penduduk_tetap extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("pentm","dm");
...
function index()
    ...
    $data['header'] = "Data Penduduk Tetap";
	$data['controller'] = "statistik_penduduk_tetap";
    

   	$content = $this->load->view("statistik_penduduk_tetap_view",$data,true);
	$this->set_title("Data Penduduk Tetap");
...
function get_data(){
 ...
 function cetak() {
	...
	$this->load->view("statistik_penduduk_tetap_cetak",$data); 	
 ...
 function cetak_l() {
    ...
    $this->load->view("statistik_penduduk_tetap_cetak_l",$data);  
  ...
  function cetak_p() {
    ...
    $this->load->view("statistik_penduduk_tetap_cetak_p",$data);  
 ...
 function pdf(){
   $data['controller'] = get_class($this);
	...
	$id_desa =	 $this->session->userdata("operator_id_desa");
	$data['l'] =  $this->dm->get_stat('l'); //$this->dm->get_data(array("jk"=>"l","id_desa"=>$id_desa))->num_rows();
	$data['p'] =  $this->dm->get_stat('p'); //$this->d
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_tetap_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_tetap_pdf_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
...
function excel(){
		...
		$this->db->where("p.hidup_mati",1)->where("p.status_kependudukan = 0",NULL,FALSE);
       ...
       // echo $this->db->last_query(); exit;
        ...
        $this->excel->getActiveSheet()->mergeCells('a'.$baris.':m'.$baris);
         $this->excel->getActiveSheet()
                ->setCellValue('A' . $baris, "DATA PENDUDUK TETAP");
       ...
       $this->format_center($arr_kolom,$baris);
        ...
        $filename = "LAPORAN DATA PENDUDUK TETAP.xls";
