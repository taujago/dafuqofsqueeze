...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Rekomendasi Surat Izin Tempat Usaha (SITU)');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SRSITU") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Rekomendasi Surat Izin Tempat Usaha (SITU)'); 
	$('#fm').form('load',row);						 
