...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Belum Pernah Menikah');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SBPM") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Belum Pernah Menikah'); 
	$('#fm').form('load',row);						 
