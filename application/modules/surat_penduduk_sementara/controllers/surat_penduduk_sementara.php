<?php 
class surat_penduduk_sementara extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("spsm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat keterangan penduduk sementara";
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->insert("surat_penduduk_sementara",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
              ...
              $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_penduduk_sementara",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_penduduk_sementara");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_penduduk_sementara sps ')
        ->join("v_penduduk2 p",'p.id_penduduk=sps.id_penduduk')
        ->where("id",$id);
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan penduduk sementara";
...
function pdf($id) {
   ...
   $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_penduduk_sementara_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
        ...
        force_download("surat keterangan penduduk sementara.doc", $html);
}
