...
$('#search_tgl_awal').datebox({  
	...
	// 			panelWidth:800,
	// 			url: '<?php echo site_url('general/penduduk_dropdown_desa_sementara/') ?>',
	// 			idField:'id_penduduk',
	// 			textField:'nama',
	// 			mode:'remote',
	// 			fitColumns:true,
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Keterangan Penduduk Sementara');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKPS") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Keterangan Pendudk Sementara'); 
	$('#fm').form('load',row);						 
