<?php 
class formulir_kk_new extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_formulir_kk_new","dm");
	...
	function index(){
		...
		$data['header'] = "Data Permohonan KK Baru (F-1.15)";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
			...
			$responce->rows[$i]['telepon']          = $result[$i]['telepon'] ; 
    ...
    function save(){
		...
		// $this->form_validation->set_rules('pindah_jenis','Jenis kepindahan','required');  
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               // $arr_data_pengikut['id_penduduk'] = $data['id_penduduk_pengikut'];
                ...
                $res = $this->db->insert("formulir_permohonan_kk_new",$data);
                ...
                $arr_data_pengikut_session = $this->session->userdata("arr_pengikut_kk_new");
                //echo "<pre>";
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    ...
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                       ...
                       $res2=  $this->db->insert("formulir_permohonan_kk_new_detail",$arr_tmp);
                    ...
                    $this->session->unset_userdata("arr_pengikut_kk_new");
                }
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
				...
				$xy = $this->db->get("formulir_permohonan_kk_new")->row();
				// $this->db->where("id_penduduk",$xy->id_penduduk);
				// $this->db->update("penduduk",array("status_kependudukan"=>"0"));
				 
				$this->db->where("id",$data['id']);
                $res = $this->db->update("formulir_permohonan_kk_new",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                ...
                $this->db->where("id",$data['id']);
                $this->db->delete("formulir_permohonan_kk_new_detail");
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    $i=0;
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                        $i++;
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("formulir_permohonan_kk_new");
    }
    echo json_encode(array("success"=>true,"pesan"=>"Berhasil dihapus"));
}
...
function cetak($id) {
   ...
   $this->db->select('*')->from('formulir_permohonan_kk_new sp')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Formulir Permohonan KK Baru";
    $data['title'] = $data['header'];

    $this->db->select("*")->from("formulir_permohonan_kk_new_detail spd")
...
function add_pengikut() {
   // print_r($_POST);
    $data = $this->input->post();
    $arr = $this->session->userdata("arr_pengikut_kk_new");

    $data['id_penduduk'] = $this->cm->get_id_penduduk($data['id_penduduk']);
    $arr[$data['id_penduduk']] = $data['hubungan'];
    $this->session->set_userdata("arr_pengikut_kk_new",$arr);
}
