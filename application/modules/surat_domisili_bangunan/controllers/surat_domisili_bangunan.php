<?php 
class surat_domisili_bangunan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sdbm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat keterangan domisi bangunan";
    ...
    function save(){
		...
		$this->form_validation->set_rules('surat_rt','RT','required|numeric');   
		$this->form_validation->set_rules('surat_tanggal','RT','required');
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_domisili_bangunan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $data['surat_tanggal'] =  flipdate($data['surat_tanggal']);
			   //  $data['tanggal'] =  flipdate($data['surat_tanggal']);
               $desa = $this->cm->data_desa();
                if($data['ttd'] == "kepala" ) {
                ...
                $res = $this->db->update("surat_domisili_bangunan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_domisili_bangunan");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_domisili_bangunan sd ')
	->join("tiger_dusun d","d.id_dusun = sd.id_dusun")
...
function pdf($id) {
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat Domisili Bangunan";
    $data['title'] = $data['header'];
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_domisili_bangunan_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
        ...
        force_download("surat keterangan domisili bangunan.doc", $html);
