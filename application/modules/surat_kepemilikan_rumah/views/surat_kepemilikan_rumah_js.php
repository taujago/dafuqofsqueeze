...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Kepemilikan Tanah');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKPR") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Kepemilikan Tanah'); 
	$('#fm').form('load',row);						 
