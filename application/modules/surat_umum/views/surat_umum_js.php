...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Keterangan Umum');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKU") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Keterangan Umum'); 
	$('#fm').form('load',row);						 
