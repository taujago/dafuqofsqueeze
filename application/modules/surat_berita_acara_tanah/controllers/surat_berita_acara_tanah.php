<?php 
class surat_berita_acara_tanah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sbamt","dm");
	...
	function index(){
		...
		$data['header'] = "Data Berita Acara Penyelesaian Masalah Tanah";
    ...
    function save(){
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');  
        ...
        if($this->form_validation->run() == TRUE) {
				...
				$data['id_penduduk_1'] = $this->cm->get_id_penduduk($data['nik1']);
				}
				if($data['sumber_data_2'] == "in") {
				$data['id_penduduk_2'] = $this->cm->get_id_penduduk($data['nik2']);
				}
                unset($data['nik1']);
				unset($data['nik2']);
				
				
                $res = $this->db->insert("surat_berita_acara_tanah",$data);
    ...
    function update() {
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_berita_acara_tanah",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_berita_acara_tanah");
    }
...
function cetak($id) {
    $this->db->select('*')->from('v_surat_berita_acara_tanah skt ')
