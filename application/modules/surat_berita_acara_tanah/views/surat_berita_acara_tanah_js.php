...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Berita Acara Penyelesaian Masalah Tanah');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SBAT") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Berita Acara Penyelesaian Masalah Tanah'); 
	$('#fm').form('load',row);						 
