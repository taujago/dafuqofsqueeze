<?php 
class surat_keterangan_hilang_surat_tanah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("skkst","dm");
	...
	function index(){
		...
		$data['header'] = "Data Surat Keterangan Kehilangan Surat Tanah";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['no_surat_rt']           = $result[$i]['no_surat_rt'] ;
			$responce->rows[$i]['rt_surat']           = $result[$i]['rt_surat'] ;
			...
			$responce->rows[$i]['ex_jk']           = $result[$i]['ex_jk'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('lebar','Lebar Bangunan','required|numeric');
		 $this->form_validation->set_rules('rt_surat','RT ','required|numeric');
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_keterangan_hilang_surat_tanah",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_keterangan_hilang_surat_tanah",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_keterangan_hilang_surat_tanah");
    }
...
function cetak($id) {
    $this->db->select('*')->from('v_surat_keterangan_hilang_surat_tanah skt ')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat Keterangan Kehilangan Surat Tanah ";
    $data['title'] = $data['header'];
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_keterangan_hilang_surat_tanah_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
        ...
        force_download("surat keterangan hilang surat tanah.doc", $html);
  }
