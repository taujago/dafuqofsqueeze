<?php 
class surat_tentang_persetujuan_mempelai extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_tentang_persetujuan_mempelai","dm");
	...
	function index(){
		...
		$data['header'] = "Data Tentang Persetujuan Mempelai";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['ayah_istri_nama']       = $result[$i]['ayah_istri_nama'] ; 
            $responce->rows[$i]['ayah_istri_nik']        = $result[$i]['ayah_istri_nik'] ; 
            $responce->rows[$i]['ayah_istri_warga_negara']   = $result[$i]['ayah_istri_warga_negara'] ;
            $responce->rows[$i]['ayah_istri_alamat']   = $result[$i]['ayah_istri_alamat'] ;
            $responce->rows[$i]['ayah_istri_desa']   = $result[$i]['ayah_istri_desa'] ;
            $responce->rows[$i]['ayah_istri_kecamatan']   = $result[$i]['ayah_istri_kecamatan'] ;
            $responce->rows[$i]['ayah_istri_kota']   = $result[$i]['ayah_istri_kota'] ;
            $responce->rows[$i]['ayah_istri_provinsi']   = $result[$i]['ayah_istri_provinsi'] ;

            $responce->rows[$i]['ibu_istri_nama']       = $result[$i]['ibu_istri_nama'] ; 
            $responce->rows[$i]['ibu_istri_nik']        = $result[$i]['ibu_istri_nik'] ; 
            $responce->rows[$i]['ibu_istri_warga_negara']   = $result[$i]['ibu_istri_warga_negara'] ;
            $responce->rows[$i]['ibu_istri_alamat']   = $result[$i]['ibu_istri_alamat'] ;
            $responce->rows[$i]['ibu_istri_desa']   = $result[$i]['ibu_istri_desa'] ;
            $responce->rows[$i]['ibu_istri_kecamatan']   = $result[$i]['ibu_istri_kecamatan'] ;
            $responce->rows[$i]['ibu_istri_kota']   = $result[$i]['ibu_istri_kota'] ;
            $responce->rows[$i]['ibu_istri_provinsi']   = $result[$i]['ibu_istri_provinsi'] ;
            ...
            $responce->rows[$i]['saksi3_nik']   = $result[$i]['saksi3_nik'] ;
            $responce->rows[$i]['saksi3_nama']   = $result[$i]['saksi3_nama'] ;
    ...
    function save(){
        ...
        $data['id'] = md5(date("Ymdhis"));
                ...
                $xx = $this->db->insert("surat_tentang_persetujuan_mempelai",$data);
     ...
     $data = $this->input->post();
        ...
        $data['istri_tanggal_lahir'] = flipdate($data['istri_tanggal_lahir']);
         $data['tanggal_surat'] = flipdate($data['tanggal_surat']);
                ...
                $xx = $this->db->update("surat_tentang_persetujuan_mempelai",$data);
                // echo $this->db->last_query();  
                // echo mysql_error();
                if($xx) {
                   //echo $this->db->last_query() . 
...
function hapus(){
    ...
    foreach($ids as $id) {
		$this->db->where("id",$id);
		$xx = $this->db->get("surat_tentang_persetujuan_mempelai")->row();
		
        $this->db->where("id",$id);
        $this->db->delete("surat_tentang_persetujuan_mempelai");
		//echo $this->db->last_query();
...
function cetak($id) {
    ...
    $data = $this->db->get("surat_tentang_persetujuan_mempelai");
    $data = $data->row_array();
     $data['controller'] = get_class($this);
    $data['header'] = "Detail Surat Kelahiran";
    $data['title'] = $data['header'];
...
function pdf($id){
    ...
    // echo "fuck off " ; exit;
       ...
       // $pdf->SetAutoPageBreak(true,10);
        ...
        $html = $this->load->view("pdf/surat_tentang_persetujuan_mempelai_pdf_view",$data,true);
...
function word($id) {
        $this->db->select('*')->from('surat_tentang_persetujuan_mempelai ')
