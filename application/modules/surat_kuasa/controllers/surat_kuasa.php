<?php 
class surat_kuasa extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_kuasa","dm");
	...
	function index(){
		...
		$data['header'] = "Data Formulir Surat Kuasa Pengisisan Biodata Penduduk WNI (F-1.03)";
   ...
   $ayah_nama = isset($_REQUEST['search_ayah_nama'])?$_REQUEST['search_ayah_nama']:"x";
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['nama']         = $result[$i]['nama']; 
             $responce->rows[$i]['alasan']         = $result[$i]['alasan']; 
               $responce->rows[$i]['alasan_pemberi']         = $result[$i]['alasan_pemberi']; 
    ...
    function save(){
        ...
        $this->form_validation->set_rules('ibu_nik','NIK Ibu','required');    
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $data['ibu_id_penduduk'] = $this->cm->get_id_penduduk($data['ibu_nik']);
                unset($data['ibu_nik']);
                 
                $res = $this->db->insert("surat_kuasa",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_kuasa",$data);
...
function pdf($id) {
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat Kuasa F-1.03";
    $data['title'] = $data['header'];
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_kuasa_pdf_view",$data,true);
