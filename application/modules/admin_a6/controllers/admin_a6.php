	...
	function get_data(){
       ...
       /*$nama_ibu = isset($_REQUEST['search_nama_ibu'])?$_REQUEST['search_nama_ibu']:"x";
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['mutasi']               = $result[$i]['mutasi'] ;
...
function excel($tahun) {
        ...
        $this->excel->getActiveSheet()->mergeCells('A'.$baris.':z'.$baris);
...
$this->excel->getActiveSheet()->mergeCells('L'.$baris.':Z'.$baris);
...
$this->excel->getActiveSheet()->mergeCells('Q'.($baris+1).':Z'.($baris+1));
...
$this->excel->getActiveSheet()
        ...
        ->setCellValue('V' . ($baris+2), "HUTAN LEBAT/ LINDUNG")
        ->setCellValue('W' . ($baris+2), "MUTASI TANAH DI DESA")
        ->setCellValue('X' . ($baris+2), "TANAH KOSONG")
        ->setCellValue('Y' . ($baris+2), "LAIN - LAIN")
        ->setCellValue('Z' . ($baris+2), "KET")
       ;  
		
        
$arr_kolom = array('a','b','c','d','e','f','g','h','i','j','k','l','m','N','o','p','q','r','s','t','u','v','w','x','y','z');
...
foreach($record->result() as $data) : 
    ...
    $this->excel->getActiveSheet()
        ...
        ->setCellValue('W' . $baris, $data->mutasi)
		->setCellValue('X' . $baris, $data->pertanian_tanah_kosong)
        ->setCellValue('y' . $baris, $data->pertanian_lain)
        ->setCellValue('z' . $baris, $data->ket)
        ;
        
         $this->format(array("arr_kolom"=>array('a','b'), "baris" => $baris, "bold"=>false));
         $this->format_center_line( array('b','c','d','e','f','g','h','i','j',
            'k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z') ,$baris);
        $baris++;
endforeach;
...
$baris+=3;
 $this->excel->getActiveSheet()         
        ...
        ->setCellValue('z' . $baris, strtoupper($this->desa2->desa).", ".date("d-m-Y"));
 $baris++;
$this->excel->getActiveSheet()         
        ->setCellValue('B' . $baris, "KEPALA ".strtoupper($this->desa2->bentuk_lembaga. " ".$this->desa2->desa) )
        ->setCellValue('z' . $baris, "SEKTERTARIS");
 $baris++;

$this->excel->getActiveSheet()         
        //->setCellValue('B' . $baris, strtoupper($this->desa2->bentuk_lembaga. " ".$this->desa2->desa) )
        ->setCellValue('z' . $baris, "");
 $baris+=3;  

$this->excel->getActiveSheet()         
        ->setCellValue('B' . $baris, $this->desa2->nama_kepala_desa )
        ->setCellValue('z' . $baris, $this->desa2->nama_sek_desa);
 $baris++;
$this->excel->getActiveSheet()
->setCellValue('z' . $baris, "NIP : " . $this->desa2->nip_sek_desa);

$baris++;
$this->excel->getActiveSheet()
->setCellValue('z' . $baris, "Pangkat  : " . $this->desa2->pangkat_sek_desa);
