<?php 
class statistik_penduduk_sementara extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("pensm","dm");
...
function index()
    ...
    $data['title'] = "Statistik Penduduk sementara";	
    $data['header'] = "Statistik Penduduk sementara";
	$data['controller'] = "statistik_penduduk_sementara";
    

   	$content = $this->load->view("statistik_penduduk_sementara_view",$data,true);
...
function get_data(){
 ...
 function cetak() {
	...
	$this->load->view("statistik_penduduk_sementara_cetak",$data); 	
  ...
  function cetak_l() {
    ...
    $this->load->view("statistik_penduduk_sementara_cetak_l",$data);  
 ...
 function cetak_p() {
    ...
    $this->load->view("statistik_penduduk_sementara_cetak_p",$data);  
 ...
 function pdf(){
    ...
    $data['header'] = "DATA PENDUDUK SEMENTARA  ";
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_sementara_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_sementara_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
        ...
        $this->db->where("p.hidup_mati",1)->where("p.status_kependudukan=1",NULL,FALSE);
       ...
       // echo $this->db->last_query(); exit;
        ...
        $this->excel->getActiveSheet()->setTitle('Data Penduduk Sementara ');
        ...
        $this->excel->getActiveSheet()->mergeCells('a'.$baris.':m'.$baris);
         $this->excel->getActiveSheet()
                ->setCellValue('A' . $baris, "DATA PENDUDUK SEMENTARA");
       ...
       $this->format_center($arr_kolom,$baris);
        ...
        $filename = "LAPORAN DATA PENDUDUK SEMENTARA .xls";
