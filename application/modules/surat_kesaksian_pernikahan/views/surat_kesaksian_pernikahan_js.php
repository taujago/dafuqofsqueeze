...
$(".atas_nama").hide();
 ...
 $("#ttd").change(function(){
	...
	$('#tgl_lahir,#tgl_pernikahan,#suami_tanggal_lahir,#istri_tanggal_lahir,#tanggal_surat,#anak_tanggal_lahir,#saksi_tanggal_lahir,#saksi2_tanggal_lahir').datebox({  
        required:false  ,
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Keterangan Kesaksian Pernikahan');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/02") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Keterangan Kesaksian Pernikahan'); 
	$('#fm').form('load',row);						 
