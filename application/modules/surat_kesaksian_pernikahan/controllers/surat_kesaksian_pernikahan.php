<?php 
class surat_kesaksian_pernikahan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_kesaksian_pernikahan","dm");
	...
	function index(){
		...
		$data['header'] = "Data Surat Keterangan Kesaksian Pernikahan";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['istri_tempat_lahir']        = $result[$i]['istri_tempat_lahir'] ; 
            $responce->rows[$i]['istri_tanggal_lahir']      = flipdate($result[$i]['istri_tanggal_lahir']) ; 
            $responce->rows[$i]['istri_agama']   = $result[$i]['istri_agama'] ;
            $responce->rows[$i]['istri_warga_negara']   = $result[$i]['istri_warga_negara'] ;
            $responce->rows[$i]['istri_pekerjaan']   = $result[$i]['istri_pekerjaan'] ;
            $responce->rows[$i]['istri_alamat']   = $result[$i]['istri_alamat'] ;
            $responce->rows[$i]['istri_rt']   = $result[$i]['istri_rt'] ;
            $responce->rows[$i]['istri_rw']   = $result[$i]['istri_rw'] ;
            $responce->rows[$i]['istri_desa']   = $result[$i]['istri_desa'] ;
            $responce->rows[$i]['istri_kecamatan']   = $result[$i]['istri_kecamatan'] ;
            $responce->rows[$i]['istri_kota']   = $result[$i]['istri_kota'] ;
            $responce->rows[$i]['istri_provinsi']   = $result[$i]['istri_provinsi'] ;

            $responce->rows[$i]['anak_nama']       = $result[$i]['anak_nama'] ; 
             $responce->rows[$i]['anak_jk']       = $result[$i]['anak_jk'] ; 
            $responce->rows[$i]['anak_nik']        = $result[$i]['anak_nik'] ; 
            $responce->rows[$i]['anak_tempat_lahir']        = $result[$i]['anak_tempat_lahir'] ; 
            $responce->rows[$i]['anak_tanggal_lahir']      = flipdate($result[$i]['anak_tanggal_lahir']) ; 
            $responce->rows[$i]['anak_agama']   = $result[$i]['anak_agama'] ;
            $responce->rows[$i]['anak_ke']   = $result[$i]['anak_ke'] ;
            $responce->rows[$i]['anak_warga_negara']   = $result[$i]['anak_warga_negara'] ;
            $responce->rows[$i]['anak_pekerjaan']   = $result[$i]['anak_pekerjaan'] ;
            $responce->rows[$i]['anak_alamat']   = $result[$i]['anak_alamat'] ;
            $responce->rows[$i]['anak_rt']   = $result[$i]['anak_rt'] ;
            $responce->rows[$i]['anak_rw']   = $result[$i]['anak_rw'] ;
            $responce->rows[$i]['anak_desa']   = $result[$i]['anak_desa'] ;
            $responce->rows[$i]['anak_kecamatan']   = $result[$i]['anak_kecamatan'] ;
            $responce->rows[$i]['anak_kota']   = $result[$i]['anak_kota'] ;
            $responce->rows[$i]['anak_provinsi']   = $result[$i]['anak_provinsi'] ;

            $responce->rows[$i]['saksi_nama']       = $result[$i]['saksi_nama'] ; 
            $responce->rows[$i]['saksi_nik']        = $result[$i]['saksi_nik'] ; 
            $responce->rows[$i]['saksi_tempat_lahir']        = $result[$i]['saksi_tempat_lahir'] ; 
            $responce->rows[$i]['saksi_tanggal_lahir']      = flipdate($result[$i]['saksi_tanggal_lahir']) ; 
           
            $responce->rows[$i]['saksi_pekerjaan']   = $result[$i]['saksi_pekerjaan'] ;
            $responce->rows[$i]['saksi_alamat']   = $result[$i]['saksi_alamat'] ;

            $responce->rows[$i]['saksi2_nama']       = $result[$i]['saksi2_nama'] ; 
            $responce->rows[$i]['saksi2_nik']        = $result[$i]['saksi2_nik'] ; 
            $responce->rows[$i]['saksi2_tempat_lahir']        = $result[$i]['saksi2_tempat_lahir'] ; 
            $responce->rows[$i]['saksi2_tanggal_lahir']      = flipdate($result[$i]['saksi2_tanggal_lahir']) ; 
    ...
    function save(){
        ...
        $this->form_validation->set_rules('istri_nik','Istri','required');
        ...
        $this->form_validation->set_rules('istri_nik','NIK Istri','callback_cek_nik');
        ...
        // $this->form_validation->set_rules('ayah_istri_nik','NIK Ayah Istri','callback_cek_nik');
        // $this->form_validation->set_rules('ibu_istri_nik','NIK Ibu Istri','callback_cek_nik');
        // $this->form_validation->set_rules('ibu_suami_nik','NIK Ibu Suami','callback_cek_nik');
        ...
        $data['istri_tanggal_lahir'] = flipdate($data['istri_tanggal_lahir']);
        $data['tanggal_surat'] = flipdate($data['tanggal_surat']);
        $data['anak_tanggal_lahir'] = flipdate($data['anak_tanggal_lahir']);
        $data['tgl_pernikahan'] = flipdate($data['tgl_pernikahan']);
        $data['saksi_tanggal_lahir'] = flipdate($data['saksi_tanggal_lahir']);
        $data['saksi2_tanggal_lahir'] = flipdate($data['saksi2_tanggal_lahir']);
        ...
        $data['id'] = md5(date("Ymdhis"));
                ...
                $xx = $this->db->insert("surat_kesaksian_pernikahan",$data);
    ...
    function update() {
     $data = $this->input->post();
        ...
        $data['saksi2_tanggal_lahir'] = flipdate($data['saksi2_tanggal_lahir']);
                ...
                $xx = $this->db->update("surat_kesaksian_pernikahan",$data);
                // echo $this->db->last_query();  
                // echo mysql_error();
                if($xx) {
                   //echo $this->db->last_query() . 
...
function hapus(){
    ...
    foreach($ids as $id) {
		$this->db->where("id",$id);
		$xx = $this->db->get("surat_kesaksian_pernikahan")->row();
		
        $this->db->where("id",$id);
        $this->db->delete("surat_kesaksian_pernikahan");
		//echo $this->db->last_query();
...
function cetak($id) {
    ...
    $data = $this->db->get("surat_kesaksian_pernikahan");
    $data = $data->row_array();
     $data['controller'] = get_class($this);
    $data['header'] = "Detail Surat Kelahiran";
    $data['title'] = $data['header'];
...
function pdf($id){
    ...
    // echo "fuck off " ; exit;
       ...
       // $pdf->SetAutoPageBreak(true,10);
        ...
        $html = $this->load->view("pdf/surat_kesaksian_pernikahan_pdf_view",$data,true);
...
function word($id) {
        $this->db->select('*')->from('surat_kesaksian_pernikahan ')
