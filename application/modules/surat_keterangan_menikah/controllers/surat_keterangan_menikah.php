<?php 
class surat_keterangan_menikah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_keterangan_menikah","dm");
	...
	function index(){
		...
		$data['header'] = "Data Surat Keterangan Nikah";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['ayah_pekerjaan']   = $result[$i]['ayah_pekerjaan'] ;
            ...
            $responce->rows[$i]['ayah_istri_pekerjaan']   = $result[$i]['ayah_istri_pekerjaan'] ;
            ...
            $responce->rows[$i]['ibu_istri_alamat']   = $result[$i]['ibu_istri_alamat'] ;
             $responce->rows[$i]['ibu_istri_pekerjaan']   = $result[$i]['ibu_istri_pekerjaan'] ;
    ...
    function save(){
        ...
        $data['id'] = md5(date("Ymdhis"));
                ...
                $xx = $this->db->insert("surat_keterangan_nikah",$data);
     ...
     $data = $this->input->post();
        ...
        $data['istri_tanggal_lahir'] = flipdate($data['istri_tanggal_lahir']);
         $data['tanggal_surat'] = flipdate($data['tanggal_surat']);
                ...
                $xx = $this->db->update("surat_keterangan_nikah",$data);
                // echo $this->db->last_query();  
                // echo mysql_error();
                if($xx) {
                   //echo $this->db->last_query() . 
...
function hapus(){
    ...
    foreach($ids as $id) {
		$this->db->where("id",$id);
		$xx = $this->db->get("surat_keterangan_nikah")->row();
...
function cetak($id) {
    ...
    $data = $this->db->get("surat_keterangan_nikah");
    $data = $data->row_array();
     $data['controller'] = get_class($this);
    $data['header'] = "Detail Surat Kelahiran";
    $data['title'] = $data['header'];
...
function pdf($id){
    ...
    // echo "fuck off " ; exit;
       ...
       // $pdf->SetAutoPageBreak(true,10);
        ...
        $html = $this->load->view("pdf/surat_keterangan_nikah_pdf_view",$data,true);
...
function word($id) {
        $this->db->select('*')->from('surat_keterangan_nikah ')
