...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Pengantar Mohon Ijin Pesta');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/300") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pengantar Mohon Ijin Pesta'); 
	$('#fm').form('load',row);		
