<?php 
class surat_ijin_pesta extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sip","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat pengantar mohon ijin pesta";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['alamat']       = $result[$i]['alamat'];  
             $responce->rows[$i]['acara']       = $result[$i]['acara'];  
              $responce->rows[$i]['undangan']       = $result[$i]['undangan'];  
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_ijin_pesta",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_ijin_pesta",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_ijin_pesta");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_ijin_pesta sd ')
...
function pdf($id) {
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan ijin pesta";
    $data['title'] = $data['header'];
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_ijin_pesta_pdf_view",$data,true);
