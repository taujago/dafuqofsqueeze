<?php 
class statistik_penduduk_pendidikan extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("stat_pddk","dm");
...
function index_utama()
    ...
    $data['title'] = "Data Penduduk Berdasarkan Pendidikan ";    
    $data['header'] = "Data Penduduk Berdasarkan Pendidikan";
    $data['controller'] = "statistik_penduduk_pendidikan";
  

 
 
    $content = $this->load->view("statistik_penduduk_pendidikan_view_utama",$data,true);
    $this->set_title("Data Penduduk Berdasarkan Pendidikan");
...
function index()
	...
	$data['controller'] = "statistik_penduduk_pendidikan";
   	...
   	$content = $this->load->view("statistik_penduduk_pendidikan_view",$data,true);
...
function get_data(){
       ...
       /* $nama  = isset($_REQUEST['search_nama'])?$_REQUEST['search_nama']:"x";
        ...
        $req_param = array (
				...
				"sort_direction" => $sord,
				"limit" => null  ,
				"id_pendidikan" => isset($_REQUEST['id_pendidikan'])?$_REQUEST['id_pendidikan']:"x",
 ...
 function cetak($id_pendidikan) {
	...
	$this->load->view("statistik_penduduk_pendidikan_cetak",$data); 	

 }

 function cetak_l($id_pendidikan) {
     $this->db->select('*')->from('v_penduduk p')
    ->where("hidup_mati","1")->where("status_kependudukan not in  ('1','3')",NULL,FALSE)
    ->where("p.id_desa",$this->session->userdata("operator_id_desa"))
    ->where("id_pendidikan",$id_pendidikan)
 ...
 function cetak_p($id_pendidikan) {
     $this->db->select('*')->from('v_penduduk p')
    ->where("hidup_mati","1")->where("status_kependudukan not in  ('1','3')",NULL,FALSE)
    ->where("p.id_desa",$this->session->userdata("operator_id_desa"))
    ->where("id_pendidikan",$id_pendidikan)
 ...
 $sql="SELECT k.`id_pendidikan`, k.`pendidikan`,
        SUM(IF(p.jk='l',1,0)) AS l,
        SUM(IF(p.jk='p',1,0)) AS p
        FROM pendidikan k
        LEFT JOIN penduduk p ON p.`id_pendidikan` = k.`id_pendidikan`
         WHERE k.deleted='0' AND p.`deleted`='0' AND status_kependudukan not in ('1','3')  AND hidup_mati ='1'
         and p.deleted='0'
        GROUP BY k.`id_pendidikan`
        ORDER BY id_pendidikan";
    $res = $this->db->query($sql);
    $data['record'] = $res;
    $this->load->view("statistik_penduduk_pendidikan_stat_cetak",$data);     
 }
 
function diag() {
    
    $data['pendidikan'] = $this->dm->stat_penduduk_pendidikan()->row();
