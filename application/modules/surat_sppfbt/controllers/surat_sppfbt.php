<?php 
class surat_sppfbt extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sppfbtm","dm");
	...
	function index(){
		...
		$data['header'] = "SURAT PERNYATAAN PENGUASAAN FISIK BIDANG TANAH
(SPPFBT)
";
...
$responce->rows[$i]['saksi1_tmp_lahir']       =  $result[$i]['saksi1_tmp_lahir'] ;
$responce->rows[$i]['saksi1_tgl_lahir']       =  flipdate($result[$i]['saksi1_tgl_lahir']) ;
$responce->rows[$i]['saksi1_umur']       =  $result[$i]['saksi1_umur'] ;
...
$responce->rows[$i]['saksi2_tmp_lahir']       =  $result[$i]['saksi2_tmp_lahir'] ;
$responce->rows[$i]['saksi2_tgl_lahir']       =  flipdate($result[$i]['saksi1_tgl_lahir']) ;
$responce->rows[$i]['saksi2_umur']       =  $result[$i]['saksi2_umur'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
            ...
            $desa = $this->cm->data_desa();
                ...
                $res = $this->db->insert("surat_sppfbt",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_sppfbt",$data);
...
function cetak($id) {
    $this->db->select('*')->from('surat_sppfbt skt ')
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_sppfbt_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
         ...
         $html = $this->load->view("pdf/surat_sppfbt_pdft_view",$data,true);

         $this->load->helper('download');
        force_download("surat kepemilikan tanah.doc", $html);
