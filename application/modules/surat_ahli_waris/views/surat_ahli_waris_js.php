...
// $( 'textarea#isi_surat' ).ckeditor();

// CKEDITOR.replace( 'textarea#isi_surat', {
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Keterangan Laporan Kebakaran');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/364") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Keterangan Laporan Kebakaran'); 
	$('#fm').form('load',row);						 
