<?php 
class surat_ahli_waris extends op_controller {
    function __construct(){
        parent::__construct();
        $this->load->model("core_model","cm");
        $this->load->model("s_ahli_waris","dm");
    ...
    function index(){
        ...
        $data['header'] = "Data surat kebakaran";
    ...
    function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['tanggal']              = flipdate($result[$i]['tanggal']) ;
             $responce->rows[$i]['tgl_kebakaran']       = flipdate($result[$i]['tgl_kebakaran']) ;
    ...
    function save(){
        ...
        // $this->form_validation->set_rules('kebakaran','kebakaran','required');       
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $data['tanggal'] = date("Y-m-d");
                 $data['tgl_kebakaran'] = flipdate($data['tgl_kebakaran']);
                ...
                $res = $this->db->insert("surat_ahli_waris_1",$data);
        ...
        if($this->form_validation->run() == TRUE) {
             $data['tgl_kebakaran'] = flipdate($data['tgl_kebakaran']);
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_ahli_waris_1",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_ahli_waris_1");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_ahli_waris_1 skt ')
...
function pdf($id) {
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_ahli_waris_pdf_view",$data,true);
