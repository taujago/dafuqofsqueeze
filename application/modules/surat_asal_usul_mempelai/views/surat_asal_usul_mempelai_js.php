...
$(".atas_nama").hide();
 ...
 $("#ttd").change(function(){
	...
	$('#tgl_lahir,#tgl_pernikahan,#suami_tanggal_lahir,#istri_tanggal_lahir,#tanggal_surat,#ayah_suami_tanggal_lahir,#ibu_suami_tanggal_lahir,#calon_tgl_lahir').datebox({  
        required:false  ,
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Pengantar Nikah');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/477") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pengantar Nikah'); 
	$('#fm').form('load',row);						 
...
function cari(){
	      $('#tt').datagrid('load',{                         
         	...
         	search_ayah_suami_nama : $('#search_ayah_suami_nama').val()   ,
         	search_ibu_suami_nama : $('#search_ibu_suami_nama').val()   ,
...
function reset_search() {
	...
	$('#search_ayah_suami_nama').val('');
	$('#search_ibu_suami_nama').val('');
...
function cetak(){
	...
	window.open('<?php echo site_url("surat_asal_usul_mempelai/pdf") ?>_'+jenis_cetak+'/'+row.id);
	}
	else {
		$.messager.alert('Error','Pilih record yang akan dicetak ','error');
	}
