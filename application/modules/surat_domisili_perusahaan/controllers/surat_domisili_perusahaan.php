<?php 
class surat_domisili_perusahaan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("suhm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat Domisili Perusahaan";
	...
	function get_data(){
		...
		//	$responce->rows[$i]['alamat']              = $result[$i]['alamat'];
            ...
            $responce->rows[$i]['nama_perusahaan']              = $result[$i]['nama_perusahaan'];
            $responce->rows[$i]['telp']              = $result[$i]['telp'];
             $responce->rows[$i]['status_bangunan']              = $result[$i]['status_bangunan'];
             $responce->rows[$i]['sk']              = $result[$i]['sk'];
               ...
               $responce->rows[$i]['lengkap']          = $result[$i]['lengkap'] ; 
                ...
                $responce->rows[$i]['pekerjaan']              = $result[$i]['pekerjaan'];
                   $responce->rows[$i]['jk']              = $result[$i]['jk'];
                     $responce->rows[$i]['alamat']              = $result[$i]['alamat'];
                       $responce->rows[$i]['pj']              = $result[$i]['pj'];
    ...
    function save(){
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
				...
				$data['tanggal_lahir'] =  !empty($data['tanggal_lahir'])?flipdate($data['tanggal_lahir']):"" ;
                ...
                // }
                // unset($data['nik']);
                $res = $this->db->insert("surat_domisili_perusahaan",$data);
     ...
     $data = $this->input->post();
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_domisili_perusahaan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_domisili_perusahaan");
    }
...
function cetak($id) {
    $this->db->select('*')->from('v_surat_usaha skt ')
...
function pdf($id) {
    $this->db->select('*')->from('surat_domisili_perusahaan ')
       // ->join("v_penduduk2 p",'p.id_penduduk=skt.id_penduduk')
        ->where("id",$id);
   ...
   $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_domisili_perusahaan_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
        ...
        force_download("surat keterangan usaha.doc", $html);
    }
