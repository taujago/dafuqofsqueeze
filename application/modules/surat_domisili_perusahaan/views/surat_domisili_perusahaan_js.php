...
$('#search_tgl_awal').datebox({  
	...
	$('#tanggal_lahir,#tgl_pernikahan,#suami_tanggal_lahir,#istri_tanggal_lahir,#tanggal_surat').datebox({  
        required:false  ,
        formatter : function(date) {
        	var y = date.getFullYear();
			var m = date.getMonth()+1;
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Keterangan Domisili Perusahaan');
			$('#fm').form('clear');
			$("#id").val('<?php echo $this->session->userdata("operator_id_desa")."."; ?>');
			url = '<?php echo site_url("$controller/save") ?>';  
			$(".atas_nama").hide();
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SDP") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Domisili Perusahaan'); 
	$('#fm').form('load',row);						 
