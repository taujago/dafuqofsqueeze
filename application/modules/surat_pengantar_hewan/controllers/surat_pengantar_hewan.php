<?php 
class surat_pengantar_hewan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sph","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat pengantar hewan";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['asal_hewan']            = $result[$i]['asal_hewan'] ;
            $responce->rows[$i]['jenis_hewan']            = $result[$i]['jenis_hewan'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('volume_hewan','Volume','required|numeric');
        $this->form_validation->set_rules('asal_hewan','Asal hewan','required');       
        $this->form_validation->set_rules('tempat_pengiriman','Tempat Pengiriman','required');    
        ...
        $this->form_validation->set_rules('ttd','Pejabat Penandatangan ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_pengantar_hewan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_pengantar_hewan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_pengantar_hewan");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_pengantar_hewan skt ')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan kepemilikan tanah";
    $data['title'] = $data['header'];
...
function pdf($id) {
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_pengantar_hewan_pdf_view",$data,true);
...
function word($id) {
    ...
    $this->load->helper('download');
        force_download("surat pengantar hewan.doc", $html);       
} 
