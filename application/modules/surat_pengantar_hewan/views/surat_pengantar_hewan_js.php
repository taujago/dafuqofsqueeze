...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Surat Pengantar Hewan');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SPKH") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pengantar Hewan'); 
	$('#fm').form('load',row);						 
