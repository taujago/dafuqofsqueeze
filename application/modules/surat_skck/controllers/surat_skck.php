<?php 
class surat_skck extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("ssm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat keterangan Memperoleh Ijin SKCK";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['keperluan']          = $result[$i]['keperluan'] ;
            ...
            $responce->rows[$i]['pejabat_kecamatan_jabatan']          = $result[$i]['pejabat_kecamatan_jabatan'] ; 
            $responce->rows[$i]['pejabat_kecamatan_nama']          = $result[$i]['pejabat_kecamatan_nama'] ; 
            $responce->rows[$i]['pejabat_kecamatan_nip']          = $result[$i]['pejabat_kecamatan_nip'] ; 
            $responce->rows[$i]['pejabat_kecamatan_pangkat']          = $result[$i]['pejabat_kecamatan_pangkat'] ; 
    ...
    function save(){
        ...
        $this->form_validation->set_rules('ttd','Pejabat Penanda Tangan','required');              
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->insert("surat_skck",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_skck",$data);
...
function cetak($id) {
    $this->db->select('*')->from('surat_skck sd ')
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_skck_pdf_view",$data,true);
