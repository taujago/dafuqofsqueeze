<?php 
class statistik_penduduk_produktif extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("penprodm","dm");
...
function index()
    ...
    $data['title'] = "Data Penduduk Usia Produktif";	
    $data['header'] = "Data Penduduk Usia Produktif";
	$data['controller'] = "statistik_penduduk_produktif";
    

   	$content = $this->load->view("statistik_penduduk_produktif_view",$data,true);
...
function get_data(){
 ...
 $this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
        ...
        where(" umur between 15 and 64 " , NULL, FALSE)
	...
	$this->load->view("statistik_penduduk_produktif_cetak",$data); 	
 ...
 $this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
    ...
    $this->load->view("statistik_penduduk_produktif_cetak_l",$data);  
 ...
 $this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
    ...
    $this->load->view("statistik_penduduk_produktif_cetak_p",$data);  
   ...
   $this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_produktif_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_produktif_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
...
// echo $this->db->last_query(); exit;
        ...
        $this->excel->getActiveSheet()->setTitle('Data Penduduk  usia produktif ');
        ...
        $this->excel->getActiveSheet()->mergeCells('a'.$baris.':m'.$baris);
         $this->excel->getActiveSheet()
                ->setCellValue('A' . $baris, "DATA PENDUDUK USIA PRODUKTIF");
       ...
       $this->format_center($arr_kolom,$baris);
        ...
        $filename = "LAPORAN DATA PENDUDUK USIA PRODUKTIF.xls";
