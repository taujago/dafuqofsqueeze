...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Rekomendasi Mendirikan Bangunan');
			$('#fm').form('clear');
			$("#id").val('<?php echo $this->session->userdata("operator_id_desa")."."; ?>');
			url = '<?php echo site_url("$controller/save") ?>';  
			$(".atas_nama").hide();
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/648") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Rekomendasi Mendirikan Bangunan'); 
	$('#fm').form('load',row);						 
