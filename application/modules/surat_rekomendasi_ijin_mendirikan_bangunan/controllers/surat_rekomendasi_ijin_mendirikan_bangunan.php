<?php 
class surat_rekomendasi_ijin_mendirikan_bangunan extends op_controller {
	...
	function index(){
		...
		$data['header'] = "Data surat Rekomendasi Izin Mendirikan Bangunan";
	...
	function get_data(){
		...
		//	$responce->rows[$i]['tgl_lahir']       = $result[$i]['tgl_lahir'] ;
            ...
            $responce->rows[$i]['sifat']              = $result[$i]['sifat']; 
            $responce->rows[$i]['lampiran']              = $result[$i]['lampiran']; 
            $responce->rows[$i]['hal']              = $result[$i]['hal']; 
    ...
    function save(){
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                // }
               // unset($data['nik']);
                $res = $this->db->insert("surat_rekomendasi_ijin_mendirikan_bangunan",$data);
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               // unset($data['nik']);
                ...
                $res = $this->db->update("surat_rekomendasi_ijin_mendirikan_bangunan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_rekomendasi_ijin_mendirikan_bangunan");
    }
...
function pdf($id) {
    $this->db->select('*')->from('surat_rekomendasi_ijin_mendirikan_bangunan skt')
       // ->join("v_penduduk2 p",'p.id_penduduk=skt.id_penduduk')
        ->where("id",$id);
   ...
   $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_usaha_pdf_view",$data,true);
