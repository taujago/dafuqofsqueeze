...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Pernyataan Akta Kelahiran');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKDOM") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pernyataan Akta Kelahiran'); 
	$('#fm').form('load',row);						 
