...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Surat Pengantar Kayu');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SPKY") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pengantar Kayu'); 
	$('#fm').form('load',row);						 
