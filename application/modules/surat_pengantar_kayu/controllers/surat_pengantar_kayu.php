<?php 
class surat_pengantar_kayu extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("spkm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat pengantar kayu";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['volume_kayu']	        = $result[$i]['volume_kayu'] ;  
    ...
    function save(){
        ...
        $this->form_validation->set_rules('volume_kayu','Volume','required|numeric');
        $this->form_validation->set_rules('asal_kayu','Asal kayu','required');       
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_pengantar_kayu",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               $desa = $this->cm->data_desa();
                ...
                $res = $this->db->update("surat_pengantar_kayu",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_pengantar_kayu");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_pengantar_kayu skt ')
...
function pdf($id) {
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_pengantar_kayu_pdf_view",$data,true);
...
function word($id) {
    ...
    $this->load->helper('download');
        force_download("surat pengantar kayu.doc", $html);       
} 
