...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Ijin Orangtua Ke Luar negeri');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKIJLN") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Ijin Orangtua Ke Luar negeri'); 
	$('#fm').form('load',row);						 
