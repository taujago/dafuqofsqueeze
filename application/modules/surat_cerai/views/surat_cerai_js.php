...
$('#search_tgl_awal').datebox({  
	...
	$('#search_tgl_akhir').datebox({  
	 ...
	 $('#perempuan_id_penduduk').combogrid({
				panelWidth:800,
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Pernyataan cerai');
			$('#fm').form('clear');
			$("#id").val('<?php echo $this->session->userdata("operator_id_desa")."."; ?>');
			url = '<?php echo site_url("$controller/save") ?>';  
			$(".atas_nama").hide();
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKCERAI") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pernyataan Cerai'); 
	$('#fm').form('load',row);						 
