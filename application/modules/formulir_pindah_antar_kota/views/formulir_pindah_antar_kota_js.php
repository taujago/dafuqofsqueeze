...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Permohonan Pindah WNI Antar Kabupaten/Kota atau Antar Provinsi (F-1.34)');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Permohonan Pindah WNI Antar Kabupaten/Kota atau Antar Provinsi (F-1.34)'); 
	$('#fm').form('load',row);						 
...
function cetak(){
	...
	window.open('<?php echo site_url("formulir_pindah_antar_kota/cetak") ?>_'+jenis_cetak+'/'+row.id);
	}
	else {
		$.messager.alert('Error','Pilih record yang akan dicetak ','error');
	}
...
function spek(){
				$('#spek').dialog('open').dialog('setTitle','Spesifikasi Formulir dan Surat Pengantar');
				$('#fm').form('clear');
