<?php 
class statistik_penduduk_kaya_miskin extends  op_controller {
...
function index()
    ...
    $data['title'] = "Statistik Penduduk Miskin";	
    $data['header'] = "Statistik Penduduk Miskin";
	$data['controller'] = "statistik_penduduk_kaya_miskin";
    

   	$content = $this->load->view("statistik_penduduk_kaya_miskin_view",$data,true);
...
function get_data(){
 ...
 $this->db->select('*')->from('v_penduduk p')
	...
	$this->load->view("statistik_penduduk_kaya_miskin_cetak",$data); 	
 ...
 function pdf(){
    ...
    $data['header'] = "DATA PENDUDUK kaya_miskin  ";
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_kaya_miskin_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_kaya_miskin_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
    ...
    $this->db->where("hidup_mati",1)->where("status_kependudukan =0 ",NULL, FALSE) 
   ->where("p.kaya_miskin = 2",NULL,FALSE);
    ...
    // ->order_by("p.nixk");
       ...
       $this->format_center($arr_kolom,$baris);
        ...
        $filename = "LAPORAN DATA PENDUDUK kaya_miskin .xls";
