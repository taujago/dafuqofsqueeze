<?php 
class formulir_pindah_datang_1_desa extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_formulir_pindah_datang_1_desa","dm");
	...
	function index(){
		...
		$data['header'] = "Data Formulir Permohonan Pindah Datang WNI Satu Desa (F-1.23)";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['kepala_keluarga_alamat']          = $result[$i]['kepala_keluarga_alamat'] ; 
            $responce->rows[$i]['kepala_keluarga_rt']          = $result[$i]['kepala_keluarga_rt'] ; 
            $responce->rows[$i]['kepala_keluarga_rw']          = $result[$i]['kepala_keluarga_rw'] ; 
			$responce->rows[$i]['kepala_keluarga_dusun']          = $result[$i]['kepala_keluarga_dusun'] ; 
            ...
            $responce->rows[$i]['tujuan_no_kk']       = $result[$i]['tujuan_no_kk']; 
    ...
    function save(){
        ...
        $this->form_validation->set_rules('status','Status No. KK Bagi Yang Pindah ','required');
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               // $arr_data_pengikut['id_penduduk'] = $data['id_penduduk_pengikut'];
                ...
                $res = $this->db->insert("formulir_pindah_datang_1_desa",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    ...
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                       ...
                       $res2=  $this->db->insert("formulir_pindah_datang_1_desa_detail",$arr_tmp);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
             $data['tanggal_kedatangan'] = flipdate($data['tanggal_kedatangan']);
				...
				$xy = $this->db->get("formulir_pindah_datang_1_desa")->row();
				// $this->db->where("id_penduduk",$xy->id_penduduk);
				// $this->db->update("penduduk",array("status_kependudukan"=>"0"));
				 
				$this->db->where("id",$data['id']);
                $res = $this->db->update("formulir_pindah_datang_1_desa",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                ...
                $this->db->where("id",$data['id']);
                $this->db->delete("formulir_pindah_datang_1_desa_detail");
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    $i=0;
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                        $i++;
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("formulir_pindah_datang_1_desa");
    }
    echo json_encode(array("success"=>true,"pesan"=>"Berhasil dihapus"));
}
...
function cetak($id) {
   ...
   $this->db->select('*')->from('formulir_pindah_datang_1_desa')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Formulir Permohonan Pindah Datang WNI Satu Desa (F-1.23)";
    $data['title'] = $data['header'];

    $this->db->select("*")->from("formulir_pindah_datang_1_desa_detail spd")
