<?php 
class surat_jual_beli_tanah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sjbt","dm");
	...
	function index(){
		...
		$data['header'] = "Data Surat Jual Beli Tanah";
    ...
    function save(){
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_jual_beli_tanah",$data);
    ...
    function update() {
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_jual_beli_tanah",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_jual_beli_tanah");
    }
...
function cetak($id) {
    $this->db->select('*')->from('v_surat_jual_beli_tanah skt ')
    ...
    $data['title'] = $data['header'];
     $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_jual_beli_tanah_pdf_view",$data,true);
