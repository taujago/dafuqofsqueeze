<?php 
class surat_keterangan_data extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("m_surat_keterangan_data","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat keterangan data";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['rumah_sakit']  = $result[$i]['rumah_sakit'] ; 
            $responce->rows[$i]['fase_kartu']  = $result[$i]['fase_kartu'] ; 
            $responce->rows[$i]['nama_kartu']  = $result[$i]['nama_kartu'] ; 
            $responce->rows[$i]['alamat_kartu']  = $result[$i]['alamat_kartu'] ; 
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $data['tanggal'] = date("Y-m-d");
                 $data['tgl_lahir_kartu'] = date("Y-m-d");
                ...
                unset($data['nik']);
                $res = $this->db->insert("surat_keterangan_data",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
             $data['tgl_lahir_kartu'] = date("Y-m-d");
                ...
                $res = $this->db->update("surat_keterangan_data",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_keterangan_data");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_keterangan_data sd ')
...
function pdf($id) {
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan Data";
    $data['title'] = $data['header'];
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_keterangan_data_pdf_view",$data,true);
