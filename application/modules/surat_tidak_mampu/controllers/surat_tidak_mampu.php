<?php 
class surat_tidak_mampu extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("stm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat keterangan tidak mampu";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['hubungan']          = $result[$i]['hubungan'] ; 
             $responce->rows[$i]['hubungan_pengikut']          = $result[$i]['hubungan_pengikut'] ; 
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
               ...
               // $arr_data_pengikut['id_penduduk'] = $data['id_penduduk_pengikut'];
                ...
                $res = $this->db->insert("surat_tidak_mampu",$data);
                ...
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    ...
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                        ...
                        $arr_tmp = array("id_penduduk"  => $id_penduduk,
                                         "hubungan"     => $hubungan,
                                          "hubungan_pengikut"     => $hubungan_pengikut,
                                         "id"           => $data['id'],
                                         "id_surat_detail" => md5(date("Ymdhis").$i)
                                         );
                       $res2=  $this->db->insert("surat_tidak_mampu_detail",$arr_tmp);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                if(isset($data['id_penduduk_pengikut'])) { 
                unset($data['id_penduduk_pengikut']); }
                if(isset($data['hubungan_pengikut'])) { 
                unset($data['hubungan_pengikut']); }
                ...
                $res = $this->db->update("surat_tidak_mampu",$data);
              ...
              //  echo  "Jumlah array " .count($arr_data_pengikut_session) ; 
                $this->db->where("id",$data['id']);
                $this->db->delete("surat_tidak_mampu_detail");
                if( is_array($arr_data_pengikut_session) and  count($arr_data_pengikut_session)  > 0 ) { 
                    $i=0;
                    foreach($arr_data_pengikut_session as $id_penduduk => $hubungan) {
                        $i++;
...
function cetak($id) {
    $this->db->select('*')->from('surat_tidak_mampu sbm ')
        ->join("v_penduduk p",'p.id_penduduk=sbm.id_penduduk')
        ->where("id",$id);
    ...
    $this->db->select("*")->from("surat_tidak_mampu_detail spd")
    ->join("v_penduduk p",'spd.id_penduduk=p.id_penduduk')
    ->where("id",$data['id']);
    $res = $this->db->get();
    $data['record'] = $res;
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_tidak_mampu_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['record'] = $res;
        ...
        force_download("surat keterangan tidak mampu.doc", $html);
   }
