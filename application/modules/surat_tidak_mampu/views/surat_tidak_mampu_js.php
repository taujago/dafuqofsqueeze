...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Keterangan Tidak Mampu');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKTM") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Keterangan Tidak Mampu'); 
	$('#fm').form('load',row);						 
...
function tambahpengikut(){
	v_nik_pengikut = $("#nik_pengikut").val();
	hubungan_pengikut	 = $("[name='hubungan_pengikut']").val();
	 
	if ( v_nik_pengikut == "")  {
		$.messager.alert('Error','Nama keluarga atau hubungan dalam keluarga harus diisi','error');
	}
	else { 
	...
	$.ajax({
		...
		type : 'POST',
		data : {'id_penduduk' : v_nik_pengikut, 'hubungan' : hubungan_pengikut},
		success : function(result) {
			 load_pengikut();
		}
	});
