<?php 
class surat_ijin_keramaian extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("sikm","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat ijin keramaian";
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['tgl_mulai']  =   flipdate($result[$i]['tgl_mulai']) ;   
            $responce->rows[$i]['tgl_selesai']  = flipdate($result[$i]['tgl_selesai']) ;
            $responce->rows[$i]['tempat_acara']  = $result[$i]['tempat_acara'] ; 
    ...
    function save(){
        ...
        $data['tgl_mulai'] = flipdate($data['tgl_mulai']);
        $data['tgl_selesai'] = flipdate($data['tgl_selesai']);
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                unset($data['nik']);
                $res = $this->db->insert("surat_ijin_keramaian",$data);

                if($res) {  

                    $data_penduduk = $this->am->get_penduduk_detail($data['id_penduduk']);

                    if(strtoupper($data_penduduk->jk) == "L") {
                        $arr = array("status_kawin"=>"d");
                    }
                    else {
                        $arr = array("status_kawin"=>"j");   
                    }
    ...
    function update() {
      ...
      //  exit;
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_ijin_keramaian",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_ijin_keramaian");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_ijin_keramaian sdj ')
        ->join("v_penduduk2 p",'p.id_penduduk=sdj.id_penduduk')
        ->where("id",$id);
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat Keterangan Ijin Keramaian";
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_ijin_keramaian_pdf_view",$data,true);
...
function word($id) {
    ...
    force_download("surat pengantar ijin keramaian.doc", $html);          
} 
}
?>