...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Pengantar Ijin Keramaian');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SIKR") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Pengantar Ijin Keramaian'); 
	$('#fm').form('load',row);						 
