<?php 
class surat_rapat_kerja extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("srp","dm");
	...
	function index(){
		...
		$data['header'] = "Data surat Rapat Kerja";
	...
	function get_data(){
        ...
        $perihal = isset($_REQUEST['search_perihal'])?$_REQUEST['search_perihal']:"x";
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['tanggal_surat']      = flipdate($result[$i]['tanggal_surat']) ;
             $responce->rows[$i]['hari']     = $result[$i]['hari'] ;
              $responce->rows[$i]['perihal']     = $result[$i]['perihal'] ;
               $responce->rows[$i]['lampiran']     = $result[$i]['lampiran'] ;
                $responce->rows[$i]['kepada']     = $result[$i]['kepada'] ;
                 $responce->rows[$i]['kepada2']     = $result[$i]['kepada2'] ;
            ...
            $responce->rows[$i]['tempat']       = $result[$i]['tempat'];  
            $responce->rows[$i]['jam']       = $result[$i]['jam'];     
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_rapat_kerja",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_belum_bekerja",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_belum_bekerja");
    }
...
function cetak($id) {
    $this->db->select('*')->from('surat_belum_bekerja sd ')
    ...
    $data = $data->row_array();
    $data['controller'] = get_class($this);
    $data['header'] = "Surat keterangan belum_bekerja";
...
function pdf($id) {
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_belum_bekerja_pdf_view",$data,true);
...
function word($id) {
    ...
    $data['title'] = $data['header'];
        ...
        force_download("surat keterangan belum bekerja.doc", $html);
