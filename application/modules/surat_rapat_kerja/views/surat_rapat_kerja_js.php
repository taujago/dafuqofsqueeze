...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Surat Rapat Kerja');
			$('#fm').form('clear');
			...
			$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/SKBK") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Surat Keterangan Belum Bekerja'); 
	$('#fm').form('load',row);		
