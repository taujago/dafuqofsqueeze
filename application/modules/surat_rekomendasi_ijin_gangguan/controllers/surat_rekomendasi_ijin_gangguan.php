<?php 
class surat_rekomendasi_ijin_gangguan extends op_controller {
	...
	function index(){
		...
		$data['header'] = "Data Rekomendasi Izin Gangguan";
    ...
    function save(){
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("surat_recomendasi_ijin_gangguan",$data);
    ...
    function update() {
       ...
       // $this->form_validation->set_rules('bidang_usaha','Bidang Usaha ','required');        
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("surat_recomendasi_ijin_gangguan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->delete("surat_recomendasi_ijin_gangguan");
    }
...
function pdf($id) {
    $this->db->select('*')->from('surat_recomendasi_ijin_gangguan ')
       // ->join("v_penduduk2 p",'p.id_penduduk=skt.id_penduduk')
        ->where("id",$id);
    ...
    $this->load->library('Pdf');
        ...
        $pdf->AddPage('P');
         ...
         $html = $this->load->view("pdf/surat_rekomendasi_ijin_gangguan_pdf_view",$data,true);
