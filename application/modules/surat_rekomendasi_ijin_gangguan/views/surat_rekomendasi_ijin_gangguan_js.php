...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Rekomendasi Izin Gangguan');
			$('#fm').form('clear');
			...
			$(".atas_nama").hide();
				...
				$.ajax({
				url: '<?php echo  site_url("general/generate_nomor/503") ?>',
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Rekomendasi Surat Ijin Gangguan'); 
	$('#fm').form('load',row);						 
